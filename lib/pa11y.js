...
async function runPa11yTest(url, options, state) {
	...
	await saveScreenCapture(url, options, state);
 ...
 */
async function saveScreenCapture(url, options, state) {
	// Generate a screen capture
	if (options.screenCapture) {
		const path = `./pa11y-ci-report/${getFileNameFromUrl(url)}.png`;
		options.log.info(
			`Capturing screen, saving to "${path}"`
		);
		try {
