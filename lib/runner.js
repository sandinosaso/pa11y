...
((exports => {
	...
	async function runPa11y(options) {
		 ...
		 */
		function processIssue(issue, index) {
			return {
				id: index,
				code: issue.code,
				type: issue.type,
				...
				context: (issue.element ? getElementContext(issue.element, issue, index) : ''),
	...
	function paintElement(element, issue, index) {
		if (issue.type === 'error') {
			const dateSpan = document.createElement('span');
			dateSpan.innerHTML = index;
			// eslint-disable-next-line max-len
			dateSpan.style = 'top:0;position:relative;float:right;color:red;font-size:18px;font-weight:bold';
	 ...
	 */
	function getElementContext(element, issue, index) {
		let outerHTML = null;
		let innerHTML = null;
		...
		if (issue.type === 'error') {
			paintElement(element, issue, index);
		}
		return outerHTML;
	}
